...
export class AudioEngine {
  ...
  // lvl scales the whole voice (0..1): the road's own crossings pass overhead
  // at anything from 9m (a flyover deck — full whoomp) to 40m (a loop's upper
  // arc — a distant murmur), and one level for both reads as a deck strapped
  // to the loop.
  archPass(speedNorm = 0.5, kind = 1, lvl = 1) {
    if (!this.ctx) return;
    const t0 = this.ctx.currentTime;
    ...
    tg.gain.setValueAtTime(((holo ? 0.050 : span ? 0.070 : 0.045) + (holo ? 0.045 : 0.05) * sn) * lvl, t0);
    ...
    // The swish sits against engine + wind + music, all of which live in its
    // band — it needs more level than instinct says, or it vanishes exactly
    // when the game is loud, which is exactly when you are going fast.
    const bodyT = holo ? 0.065 : span ? 0.13 : 0.09;
    f.frequency.setValueAtTime(b0, t0);
    f.frequency.exponentialRampToValueAtTime(b1, t0 + bodyT);
    f.Q.value = holo ? 3.0 : 1.5;
    const g = this.ctx.createGain();
    g.gain.setValueAtTime(((holo ? 0.038 : span ? 0.075 : 0.045) + (holo ? 0.045 : 0.065) * sn) * lvl, t0);
    ...
    if (holo) this.blip(2100 + 500 * sn, 0.05, { type: 'triangle', gain: (0.032 + 0.032 * sn) * lvl, slideTo: 1400 });
    else if (span) this.blip(95 + 40 * sn, 0.11, { type: 'sine', gain: (0.075 + 0.045 * sn) * lvl, slideTo: 52 });
    else this.blip(115 + 55 * sn, 0.05, { type: 'sine', gain: (0.03 + 0.025 * sn) * lvl, slideTo: 68 });
  }
