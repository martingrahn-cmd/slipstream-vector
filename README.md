...
And the rivals **speak their lines**: `tools/generate-voices.mjs`
batch-generated ElevenLabs TTS for the whole line bank (voices cast in
`pilot-expression-lab.html`), and the feed plays each clip through a
radio-comms chain — band-limit EQ, a static bed, squelch keying open and
closed — one voice at a time, ducking the music while a rival talks.
...
assets/voice/              generated rival VO clips + manifest (per pilot/bucket)
assets/sfx/                generated one-shot SFX layer + manifest
tools/generate-voices.mjs  idempotent ElevenLabs batch TTS for the LINES bank
tools/generate-sfx.mjs     idempotent ElevenLabs sound-generation batch (SFX layer)
pilot-expression-lab.html  pilot face/expression + voice-casting lab
sfx-lab.html               A/B the in-game synth vs ElevenLabs; exports sfx-config
...
fanfares and menu blips. `src/core/audio.js`. The big one-shots additionally
carry a **baked ElevenLabs layer** (`assets/sfx/`, generated by
`tools/generate-sfx.mjs` from prompts auditioned in `sfx-lab.html`): the
synth keeps the low-end attack synced with hitstop and rumble, the clip adds
the organic debris/crackle/exhaust a synth can't fake — and any missing clip
just means the full synth voice plays instead. The continuous beds (engine,
wind, shield hum, scrape) stay purely procedural: they follow game state
every frame, which a baked file can't. **Music and SFX have separate
...
soundtrack, **pilot voices** over a radio-comms chain (ElevenLabs, cast in the
lab), a **baked ElevenLabs one-shot layer** over the synth (sfx-lab.html →
